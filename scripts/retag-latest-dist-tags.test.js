...
function writeFakeNpm(binDir, options = {}) {
  ...
  const currentTags = options.currentTags || { latest: '2.1.177-1', candidate: '2.1.178' };
...
function runRetagTest(options = {}) {
  ...
  const env = {
    ...
    CLAUDE_TERMUX_EXPECTED_PREVIOUS_AUDITED_VERSION: options.previousAuditedVersion || '2.1.177-1',
  };
...
test('retag latest aborts when the registry tags drift', () => {
  const result = runRetagTest({
    currentTags: { latest: '2.1.165', candidate: '2.1.161-2' },
  });
